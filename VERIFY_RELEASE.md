...
A checksum proves that the ZIP matches the checksum text you received. It does not prove who published either file. For publisher authenticity, obtain the checksum from a trusted, separately controlled location such as the owner's HTTPS portfolio and GitHub Release, and compare both copies.
...
A strict PASS covers manifest matching, secret scanning, dangerous-code scanning, allowed network origins and routes, dry-run defaults, the tracked kill switch, immutable trade limits, exact installed dependency versions, Python compilation, 113 regression and security tests, `pip check`, Ruff, and `pip-audit`.
